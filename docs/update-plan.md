...
### 轮次 13 — 2026-08-23（云存储完善：修复/页面化/压缩档位 + UI 微调）

> 方案 `docs/image-storage-redesign.md` 已落地完整（git 3.0.37~3.0.47）。

**云存储迭代**：
- 3.0.37：修复 4 个 ArkTS 错误（PendingUpload interface / HMAC SymKey / cosConfig 初始化顺序）
- 3.0.38：**自定义访问域名**（中国大陆 bucket 默认域名受限；签名 host 与实际域名一致）
- 3.0.39：联网核实腾讯官方 SDK——APPID 无需单独传（bucket name-appid）；**content-type 必须签名**（修复 PUT 403）
- 3.0.40：`HmacSha1` 纯 ArkTS 实现（设备 cryptoFramework 不支持 HMAC|SHA1，code 401；Node 黄金向量 6/6）
- 3.0.41：上传全部图片（历史补传 + 失败重试 + 可读 key `records/{日期}/{文件名}_{idx}.jpg`）
- 3.0.42~3.0.46：**分批提交**（服务层加固 / 配置页面化 / 上传链路 / 查看原图）；总开关、去重（云端被删自动补传）、进度条、上传后删原图开关、修改/删除同步删云端、老图迁移（上传后压缩替换指向预览图）、刷新单例内存（防显示空）
- 3.0.47：UI 微调——子页面返回按钮 44×44 + 图标 24 统一；撤销条移顶部

**验证清单（云存储）**：
- [x] 测试连接成功（用户实机）
- [x] 上传全部/去重/进度/总开关/压缩档位/查看原图 全链路已实现（待用户回归）
- [ ] COS 控制台删除对象后「上传全部」应补传
