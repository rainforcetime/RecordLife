...
- ⚠️ 仍有意保留的硬编码（重构 B2 残余）：`TagModel` 预设标签 name（'工作'/'生活' 等 10 个——数据层，含 id/color，与用户自定义标签同结构，未国际化）、`MorePageHelper` `throw new Error('读取/写入/复制/创建目录失败')`（异常消息，拼接进失败 toast，错误路径开发导向）。其余 UI 文本（主题/性别显示映射、ViewModel 默认值、选择器 options、toast、时间单位、版本号、存储失败兜底）已于轮次 14/15 全部资源化；`MorePageBackupHandler` 部分 `getStringByNameSync('...')` 直接按名取（key 名未走 `$r()` 编译期校验）。
